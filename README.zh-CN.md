...
> **树莓派 / ARM：** 挖矿器**和** Kick 边车容器都同时为 `linux/amd64`
> **和** `linux/arm64` 发布。边车容器按架构选择浏览器：amd64 用 Google
> Chrome，arm64 用 Debian 的 **Chromium** —— 它是针对系统 FFmpeg 构建的，
> 因此仍然带有解码 Kick IVS 流所需的专有 **H.264/AAC 编解码器**。
> Kick-on-ARM **已确认可用**，但 arm64 边车容器**资源占用很高**
> （每个运行中的边车约 4 GB 内存，在小型设备上大约只能同时跑 2 个），
> 所以低内存的树莓派很难同时观看超过几个 Kick 账户。
